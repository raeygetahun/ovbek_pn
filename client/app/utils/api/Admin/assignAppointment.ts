...
const assignAppointment = async (email: string, applicationDate: Date, slotId: string): Promise<{ success: boolean, error?: string }> => {
    const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL;
    try {
        const response = await fetch(`${backendUrl}/api/admin/assign-time-slot`, {
            method: 'POST',
